...
const os = require('os');
const { listSkillIds, readSkill, tokenize, unique } = require('../core/skill-utils');
...
const SKILLS_SOURCE_DIR = path.join(PACKAGE_ROOT, 'plugins');
const CATALOG_PATH = path.join(PACKAGE_ROOT, 'registry.json');
const BUNDLES_PATH = path.join(PACKAGE_ROOT, 'packages.json');
const ALIASES_PATH = path.join(PACKAGE_ROOT, 'shortcuts.json');

// Define destinations
const GLOBAL_SKILLS_DIR = path.join(os.homedir(), '.gemini', 'antigravity', 'plugins');
const LOCAL_SKILLS_DIR = path.join(process.cwd(), '.agent', 'plugins');
