...
const {
  ...
  tokenize,
  unique,
} = require('../core/skill-utils');

const ROOT = path.resolve(__dirname, '..');
const SKILLS_DIR = path.join(ROOT, 'plugins');
...
function buildCatalog() {
  ...
  fs.writeFileSync(path.join(ROOT, 'registry.json'), JSON.stringify(artifacts.catalog, null, 2));
  fs.writeFileSync(path.join(ROOT, 'REGISTRY.md'), artifacts.catalogMarkdown);
  fs.writeFileSync(path.join(ROOT, 'packages.json'), JSON.stringify(artifacts.bundles, null, 2));
  fs.writeFileSync(path.join(ROOT, 'shortcuts.json'), JSON.stringify(artifacts.aliases, null, 2));
