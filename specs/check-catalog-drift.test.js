...
const { findDrift, readDiskArtifacts } = require('../build-tools/check-catalog-drift');
const { computeArtifacts } = require('../build-tools/build-catalog');
const { tmpDir, rmDir } = require('./helpers');

const ROOT = path.resolve(__dirname, '..');
const SCRIPT = path.join(ROOT, 'build-tools', 'check-catalog-drift.js');
...
test('findDrift detects a drifting artifact without touching any file', () => {
  ...
  assert.ok(drift.includes('registry.json'), `expected registry.json drift, got ${JSON.stringify(drift)}`);
});
...
test('findDrift detects REGISTRY.md drift (markdown path)', () => {
  const fresh = computeArtifacts();
  const disk = readDiskArtifacts();
  const tampered = JSON.parse(JSON.stringify(disk));
  ...
  assert.ok(findDrift(fresh, tampered).includes('REGISTRY.md'), 'markdown tamper should be detected');
});

test('readDiskArtifacts throws a helpful error when artifacts are missing', () => {
  const dir = tmpDir(); // empty: no registry.json etc.
  try {
